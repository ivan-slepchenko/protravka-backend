...
export const createOrderRecipe = (order: Order) => {
    ...
    if (
        ...
        const productRecipes = productDetails.map((productDetail) => {
            ...
            const seedsKgToUseForSlurryEstimation =
                seedsToTreatKg + (extraSlurry / 100) * seedsToTreatKg;
        ...
        const slurryTotalMltoU_KS = productRecipes.reduce(
            (sum, recipe) => sum + recipe.rateMltoU_KS,
            0,
        );
        const slurryTotalGToU_KS = productRecipes.reduce(
            (sum, recipe) => sum + recipe.rateGrToU_KS,
            0,
        );
        const slurryTotalMlTo100Kg = productRecipes.reduce(
            (sum, recipe) => sum + recipe.rateMlTo100Kg,
            0,
        );
        const slurryTotalGTo100Kgs = productRecipes.reduce(
            (sum, recipe) => sum + recipe.rateGrTo100Kg,
            0,
        );
        const slurryTotalMlRecipeToMix = productRecipes.reduce(
            (sum, recipe) => sum + recipe.mlSlurryRecipeToMix,
            0,
        );
        const slurryTotalGrRecipeToMix = productRecipes.reduce(
            (sum, recipe) => sum + recipe.grSlurryRecipeToMix,
            0,
        );
        ...
        console.log('Recipe totalCompoundsDensity:', totalCompoundsDensity);
